...
namespace SKPortfolio\Forms;

use PHPMailer\PHPMailer\SMTP;
use PHPMailer\PHPMailer\PHPMailer;
use PHPMailer\PHPMailer\Exception as PHPMailerException;
use SKPortfolio\Exceptions\Exception;
use SKPortfolio\Lang;

final class Contact extends ReCAPTCHAV2 {
  /**
  ...
  public function __construct() {
    ...
    $this->add('name', function (string $x): string {
      ...
      if (empty($x) || !preg_match("/^[a-zàâçéèêëîïôûùüÿñæœ \-.]*$/i", $x)) {
  ...
  public function submit(): void {
    ...
    try {
      ...
      $this->result = Lang::get('mail_delivered');
  ...
  private function sendConfirmationToUser(PHPMailer $mail): void {
    ...
    $mail->Subject = Lang::get('mail_contact_form_subject_confirmation');
    $mail->Body = Lang::get('mail_contact_form_message_confirmation');
    $mail->send();
  }
}

...
const REGEX_ALL_TAGS = new RegExp(
  [
    // Image tag - captures src attribute from img elements
    `<img\\s+[^>]*src=["'](\\/[^"']+\\.(?:ico|jpeg|png|svg))["'][^>]*>`,
    // Style tag - captures href attribute from link rel="stylesheet" elements
    `<link\\s+[^>]*rel=["']stylesheet["'][^>]*href=["'](\\/[^"']+\\.css)["'][^>]*>`,
    // Script tag - captures src attribute from script elements
    `<script\\s+[^>]*src=["'](\\/[^"']+\\.(?:m?js))["'][^>]*><\\/script>`,
    // ES module import - captures module path from import statements
    `import\\s+[^'"]+\\s+from\\s+['"](\\/[^'"]+\\.mjs)['"]`,
    // SSI include tag - captures virtual path from Server Side Include tags
    `<!--#include\\s+virtual=["']([^"']+)["']\\s*-->`,
    // Meta og:image tag - captures image path from Open Graph meta tags
 ...
 */
class Asset {
  #pathname = null; // Original asset path
  #mtime = 0; // Last modification time
  #hasChange = false; // Whether asset has changed since last build
  #outname = null; // Generated output filename
  #outPathname = null; // Full output path
   ...
   * @param {string|null} [outname=null] - The output filename
   */
   ...
   *
   * @returns {string|null} The output pathname
   */
  get outPathname() {
   ...
   *
   * @param {string|null} outname - The output filename
   * @returns {null}
   */
  #setOutname(outname) {
    if (!outname) return null;
    this.#outname = outname;
    // Preserve directory structure in output path
  ...
  async build(srcDir, outDir) {
    ...
    if (/\.(m?js|css)$/.test(src)) {
      ...
      const result = await Bun.build({
        ...
        sourcemap: "none",
        target: "browser",
        minify: false, // We'll minify manually for better control
        splitting: false,
      });
   ...
   *
   * @returns {{mtime: number, outname: (string|null)}} The asset configuration:
 ...
 */
class PageBuilder {
  #code = null; // Page HTML content
  #assets = []; // List of assets referenced by this page

  #pathname = null; // Original page path
  #mtime = 0; // Last modification time
  #hasChange = false; // Whether page has changed since last build
  #outPathname = null; // Output path (converts .shtml to .html)
   ...
   *
   * @returns {string|null} The page code
   */
  get code() {
    return this.#code;
  }
  ...
  async #preprocessSSI(srcDir, manifest, allAssets) {
    ...
    const replacements = await Promise.all(
      includes.map(async ({ pathname, match }) => {
        ...
        // Build include without writing to disk (outDir = null)
   ...
   *
   * @param {string} srcDir - The source directory
   * @param {string|null} outDir - The output directory
   * @param {Object} manifest - The build manifest
   * @param {Object} allAssets - All assets in the project
   * @returns {Promise<void>}
   */
 ...
 *
 * @param {string|null} manifestPath - The path to the manifest file
 * @returns {Promise<{version: string|null, pages: Object, assets: Object}>}
 ...
 * @param {string} srcDir - The source directory
 * @param {string} outDir - The output directory
 * @param {string} version - The build version string
 * @param {boolean} [force=false] - If true, forces a full rebuild
 * @returns {Promise<void>}
 */
export async function pack(srcDir, outDir, version, force = false) {
  // Ensure output directory exists
  if (!(await exists(outDir))) await mkdir(outDir);

  const manifestPath = resolve(
    join(outDir, "..", `${basename(srcDir)}-manifest.json`),
  );

  // Load previous build state for incremental builds
  const manifest = await loadManifest(force ? null : manifestPath);
  ...
  await Promise.all(
    allAssetPathnames.values().map(async (pathname) => {
      ...
      await asset.build(srcDir, outDir);
  ...
  await Promise.all(
    pages.map((page) => {
      ...
      return page.build(srcDir, outDir, manifest, allAssets);
    }),
  );
  ...
  if (pages.some((page) => page.hasChange)) {
    ...
    await cleanup(outDir, pages, allAssets);
